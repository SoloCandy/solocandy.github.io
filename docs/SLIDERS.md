...
| Ride Height F/R (RIDE HEIGHT → CG toggle) | 2–24in / 5–61cm | Alternative to manual CG Height entry — estimates CG height from ride height (as shown on Forza's stat screen) + tyre radius. Also drives a SAG vs LOAD chart (static sag from ride Hz alone, `g/(2π·Hz)²`, plotted against vertical load factor with a bottom-out marker where sag crosses the entered ride height). Uncalibrated heuristic — see README Calibration notes and [KNOWN_ISSUES.md](KNOWN_ISSUES.md) | None directly — feeds `ch.cgHeight` |
...
| ARB F / ARB R (Stiffness Mode MAN) | Forza: 1–`lim.arb` clicks (65 HORIZON / 40 MOTORSPORT). BEAMNG: 0–60000 N·m/rad | Direct ARB values, bypasses budget/split entirely. The max tracks the game mode rather than being fixed at 65 — `computeTune` clamps to `lim.arb`, so a fixed ceiling let you type values MOTORSPORT would silently discard. In BEAMNG the field carries roll stiffness, not clicks, and has no ceiling; switching game modes converts the stored value by `ARB_RS_SCALE·track²` in both directions | More rear-relative → OVERSTEER |
